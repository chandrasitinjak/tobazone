    ...
    <div class="card-body">
      <div v-for="(merchant, id) in merchants">
        <div>
          ...
          <span class="float-right font-weight-bold" style="color: #ff8415">{{
            merchant.name
          }}</span>
        </div>
        <br />
        <div>
          ...
          <span class="float-right font-weight-bold"
            >Rp. {{ formatPrice(merchant.totalProductCost) }}</span
          >
        </div>
        <div>
          <span class="">
            <div class="btn-group btn-block">
              <button
                type="button"
                class="btn btn-sm btn-block btn-primary dropdown-toggle"
              ...
              <div class="dropdown-menu">
                <button
                  ...
                  class="dropdown-item"
                  @click="changeKurir(id, idx)"
                  href="#"
                >
                  {{ cost.shipping_name }}: Rp.{{
                    formatPrice(cost.cost_shipping.cost)
                  }}
                </button>
              </div>
            </div>
        ...
        <div>
          ...
          <span class="float-right font-weight-bold"
            >Rp. {{ formatPrice(merchant.totalShippingCost) }}</span
          >
        </div>
        ...
        <div
          style="border-bottom: 1px #b4b4b4 solid; margin: 10px 0px 10px 0px"
        ></div>
        Total Pembayaran
        <div class="float-right" style="color: #ff8415">
          Rp
          {{
            formatPrice(merchant.totalProductCost + merchant.totalShippingCost)
          }}
        </div>
        <div>
          <span>Kurir</span>
          <span class="float-right font-weight-bold"
            >{{ merchant.courier_used }}({{ merchant.estimate_waktu }})</span
          >
        </div>
...
export default {
  ...
  methods: {
    ...
    changeKurir(id, idx) {
      this.merchants[id].totalShippingCost = this.merchants[id].all_shipping[
        idx
      ].cost_shipping.cost;
      this.merchants[id].estimate_waktu = this.merchants[id].all_shipping[
        idx
      ].cost_shipping.etd;
      this.merchants[id].courier_code = this.merchants[id].all_shipping[
        idx
      ].shipping_code;
      this.merchants[id].courier_used = this.merchants[id].all_shipping[
        idx
      ].shipping_name;
